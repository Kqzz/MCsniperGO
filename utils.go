	...
	for i, accStr := range accSlice {
		...
		if err != nil {
			if !errors.Is(err, errAccIgnored) {
				log(`got error "%v" while loading acc on line %v`, 2, err, i+1)
			}
			continue
		}
...
func announceSnipe(username, auth string, account *mcgo.MCaccount) error {
	...
	if res.StatusCode != 204 {
		log("got unknown status code while announcing snipe: %v", 2, res.StatusCode)
	}
		...
		case mcgo.MsPr, mcgo.Ms:
			{
				...
				log("authenticating %s through ms auth", 0, accID(acc))
			}
		case mcgo.Mj:
			{
				...
				log("authenticating %s through mojang auth", 0, accID(acc))
			}
		}
	} else {
		log("authing %s through manual bearer", 0, accID(acc))
	}
